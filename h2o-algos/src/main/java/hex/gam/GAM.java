...
public class GAM extends ModelBuilder<GAMModel, GAMModel.GAMParameters, GAMModel.GAMModelOutput> {
  ...
  IcedHashSet<Key<Frame>> _validKeys = null;
  ...
  private class GAMDriver extends Driver {
    ...
    public Frame buildGamFrame() {
      ...
      Vec weightsVec = null;
      if (_parms._weights_column != null) // needs to be the vec at the end before the response column
      ...
      try {
        ...
        if (newValidFrame != null) {
          if (_cv_on) {
            ...
            _validKeys.addIfAbsent(newValidFrame._key);
          } else {
            DKV.remove(newValidFrame._key);
          }
        }
        if ((_validKeys != null) && (!_cv_on)) {  // add valid keys here to be removed later
          model._validKeys = _validKeys.clone();
          model.update(_job);
        }
        model.unlock(_job);
    ...
    void copyGLMCoeffs(GLMModel glm, GAMModel model, DataInfo dinfo) {
      int totCoefNumsNoCenter = glm.coefficients().size()+_parms._gam_columns.length;
